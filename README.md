...
3. You will need to have Consul installed locally if you want to run the script provided from your local machine. The reason is because the script will run the **consul keygen** command which requires Consul. It will then store the output of the command onto a Vault KV store. 
 
If you do not have Consul locally, you can edit the script to explicitly set the gossip key:
```
